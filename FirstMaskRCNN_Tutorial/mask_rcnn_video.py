...
# load the COCO class labels or Mask R-CNN was trained on 
...
# derive the paths to the Mask R-CNN train on the COCO dataset (90 classes)
weightsPath = os.path.sep.join([args["mask_rcnn"],
                                "frozen_inference_graph.pb"])
configPath = os.path.sep.join([args["mask_rcnn"],
                                "mask_rcnn_inception_v2_coco_2018_01_28.pbtxt"])

# load our Mask R-CNN train on the COCO dataset (90 classes)
# from disk
...
try:
    prop = cv2.cv.CV_CAP_PROP_FRAM_COUNT if imutils.is_cv2() \
        else cv2.CAP_PROP_FRAM_COUNT
...
while True:
    try:
        ...
        for i in range(0, boxes.shape[2]):
            # extract the class ID of the etection along with the
            ...
            if confidence > args["confidence"]:
                ...
                # extract the pixel-wise segmentaton for the object,
                # resize the mask such that it's the same dimensions of
                # the bounding box, and then finally threshold the create
                # a *binary* mask
                ...
                # then create transparent overlay by blending the color
                # with the ROI 
                ...
                # the instance segmentation on the rame
